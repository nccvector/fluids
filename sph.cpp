...
const float h = 2.5f;     // Particle interaction radius
float k = 500.0f;           // Stiffness constant (Pressure Multiplier)
float rho0 = 1.0f;        // Target density
const float sigma = 0.0f; // Some viscosity factor
const float beta = 0.0f;  // Some viscosity factor
...
void ApplyInput(std::vector<Particle> &particles, glm::vec2 mousePosition,
                glm::vec2 mouseDelta) {
  for (int i = 0; i < NUM_PARTICLES; i++) {
    float distance = glm::length(mousePosition - particles[i].position);
    if (distance < 10.0f) {
      particles[i].velocity =
          20.0f * mouseDelta / (glm::length(mouseDelta) + 0.0001f);
    }
  }
}
  ...
  for (int i = 0; i < NUM_PARTICLES; i++) {
    ...
    for (int j = 0; j < NUM_PARTICLES; j++) {
      if (i == j)
        continue;
      float distance =
          glm::length(particles[j].position - particles[i].position);
      if (distance > h)
        continue;
...
void Simulate(std::vector<Particle> &particles) {
  ...
  for (auto &particle : particles) {
    ...
    if (particle.position.y - h <= 0 || particle.position.y + h >= BOUNDS.y) {
      ...
      particle.position.y =
          std::clamp(particle.position.y, 0 + h + BOUNDARY_EPSILON,
    ...
    if (particle.position.x - h <= 0 || particle.position.x + h >= BOUNDS.x) {
      ...
      particle.position.x =
          std::clamp(particle.position.x, 0 + h + BOUNDARY_EPSILON,
  ...
  while (!WindowShouldClose()) {
    ...
    for (auto &particle : particles) {
      ...
      DrawCircle(particle.position[0] * VIS_SCALE.x,
                 particle.position[1] * VIS_SCALE.y, h / 8.0f * VIS_SCALE.x,
                 {(unsigned char)finalColor.x, (unsigned char)finalColor.y,
                  (unsigned char)finalColor.z, 255});
    }

    DrawCircleLinesV(Vector2{mousePosition.x, mousePosition.y},
    ...
    ImGui::SliderFloat("Acceleration Damping", &ACCELERATION_DAMPING, 0.0f,
                       1.0f);
    ImGui::End();
